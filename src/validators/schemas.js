...
export const propertySchema = z.object({
  body: z.object({
    ...
    slug: z.string().trim().max(140).optional().default(""),
    seoTitle: z.string().trim().max(90, "SEO title should stay under 90 characters. Recommended length is 50-70 characters.").optional().default(""),
    metaDescription: z.string().trim().max(180, "Meta description should stay under 180 characters. Recommended length is 120-160 characters.").optional().default(""),
    ...
    listingType: z.enum(["", "sale", "rent"]).optional().default(""),
    ...
    plotArea: z.coerce.number().min(0).optional().default(0),
    measurement: z
      .object({
    ...
    images: z.array(mediaString).optional().default([]),
    imageAltTexts: z.array(z.string().trim().max(180)).optional().default([]),
    media: z.array(mediaAssetSchema).optional().default([]),
    description: propertyDescription.min(20, "Property description is required and should be at least 20 characters."),
    ...
    assignedSupervisor: assignableId.optional().nullable(),
    ...
    lastModifiedAt: z.union([z.coerce.date(), z.literal(""), z.null()]).optional().default(null),
    source: z.enum(["home", "pricing", "admin_added", "supervisor_added", "seller_owner"]).default("pricing"),
  }).refine((value) => value.locationRef || value.locationId || value.locationSlug, {
    message: "Master location is required",
    path: ["locationRef"],
  }).refine((value) => Boolean(value.image || value.gallery?.length || value.images?.length), {
