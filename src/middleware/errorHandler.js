...
import { CMS_VIDEO_MAX_MB } from "../utils/localVideoStorage.js";
...
export function errorHandler(error, req, res, _next) {
  ...
  const fileSizeMessage = req.originalUrl?.includes("/uploads/home-video")
    ? `Video is too large. Maximum allowed size is ${CMS_VIDEO_MAX_MB}MB.`
    : "Image is too large. Maximum allowed size is 15MB per image.";

  const payload = {
    success: false,
    message: error instanceof ZodError && req.originalUrl?.includes("/api/admin/properties") ? "Property validation failed" : error instanceof ZodError ? "Validation failed" : error.code === "LIMIT_FILE_SIZE" ? fileSizeMessage : error.type === "entity.too.large" ? "Request is too large. Please upload media through the uploader." : error.message || "Internal server error",
  };
